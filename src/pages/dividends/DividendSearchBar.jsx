...
import DividendSearchResults from "./DividendSearchResults.jsx";
import StockNameThreeScene from '../../components/StockNameThreeScene.jsx'
import axios from 'axios'

export default function DividendSearchBar() {
    const searchBar = useRef();
    const [searchResults, setSearchResults] = useState(null)


    async function search() {
        let response = await axios.get(`http://localhost:3000/nameSearch/${searchBar.current.value}`)
    ...
    return (
        <div style={{ display: 'flex', flex: '1', flexDirection: 'column', alignItems: 'center', marginLeft: '240px'}}>
            <div style={{display: 'flex', flexDirection: 'column', boxShadow: '0px 4px 16px rgba(0, 0, 0, 0.1)', borderRadius: '10px', padding: '30px', justifyContent: 'center'}}>
                <h1 style={{ textAlign: 'center'}}>Dividend Info Search</h1>
            ...
            {searchResults ? <DividendSearchResults data={searchResults} /> : <StockNameThreeScene/>}
        </div>
    )
}